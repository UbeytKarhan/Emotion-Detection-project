...
emotions = {
    ...
    4: ['uzuntulu', (255,0,0), (255,255,255)],
...
# webcam okuma nesnesi oluşturulur
vid = cv2.VideoCapture(0)

while(True):

    # cam'den frame okunur.
    ret, frame = vid.read()

    # yapay zeka modeline vermek için ön işleme yapılır
    image = detection_preprocessing(frame)

    # yapay zeka sonucu alınır
    result = inference(image)

    # sonuç ekranda gösterilir
    cv2.imshow('frame', result)

    # q ya basılana veya program durdularana kadar webcam okunmaya devam edilir.
...
def process_video():
    ...
    while True:
        ...
        # İşleme yapmak için frame'i gerekli şekilde dönüştürün
        image = detection_preprocessing(frame)

        # Emotion detection işlemini gerçekleştirin
        result = inference(image)
